...
function getCreativeGenerationConfig() {
    return {
        temperature: 0.7,
        topP: 0.9,
        topK: 40,
...
function getDeterministicGenerationConfig() {
    return {
        temperature: 0.3,
        topP: 0.8,
        topK: 40,
...
function getMarketingSchema() {
    return {
        ...
        properties: {
            title: {
                type: "string",
                description: "Create an irresistible, SEO-optimized title that sparks wanderlust. Use power words like 'Ultimate', 'Exclusive', 'Breathtaking'. Include destination and unique selling points.",
            },
            keywords: {
                type: "array",
                items: { type: "string" },
                description: "SEO-focused keywords that travelers search for: destination names, activities, experiences, attractions.",
            },
            highlights: {
                type: "array",
                items: { type: "string" },
                description: "Irresistible selling points that create urgency and desire. Include exclusive experiences and luxury amenities.",
            },
            description: {
                type: "string",
                description: "Write a captivating 200+ word description that tells a story and paints a vivid picture. Start with an emotional hook, describe transformative experiences, and end with a compelling call-to-action.",
            },
            audioSummary: {
                type: "string",
                description: "A conversational audio script summarizing the package in simple words, alternating between Speaker 1 and Speaker 2, highlighting key attractions, activities, and experiences to engage listeners.",
            },
            summary: {
                type: "string",
            ...
            itinerary: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        description: {
                            type: "string",
                            description: "Write immersive 100+ word descriptions for each day.",
                        },
                        meals: {
                            type: "array",
                            items: { type: "string" },
                        ...
                        activityDetails: {
                            ...
                            items: {
                                ...
                                properties: {
                                    ...
                                    activityDetails: {
                                        type: "array",
                                        description: "Ordered roughly as in itinerary. Include only MAIN events (tours, attractions, safari, etc.). Skip non-main items listed in the schema description.",
                                        items: {
                                            type: "object",
                                            ...
                                            properties: {
                                                ...
                                                description: {
                                                    ...
                                                    minLength: 50,
                                                    maxLength: 600,
                                                    description: "One paragraph (3-4 short sentences) describing what guests can expect. No markdown, no lists, no extra quoting. Keep concise.",
                                                },
                                            },
                                        },
                                    },
            ...
            transportation: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        vehicle: {
                            type: "string",
                            description: "Specify premium options or vehicle types like 'SUV', 'Sedan', 'Private Yacht'.",
                        },
                        details: {
                            type: "string",
                            description: "Highlight comfort features, scenic routes, and convenience.",
                        },
                        shared: {
                            type: "boolean",
    ...
    try {
        ...
        try {
            if (packageData.audioSummary) {
                ...
                console.log(`Audio generated and uploaded. URL: ${audioResult.audioUrl}`);
                console.log(`Audio generation cost: $${audioResult.cost.totalCost.toFixed(4)}`);
            }
        }
        catch (audioError) {
        ...
        console.log(`Total time for processing '${path.basename(localPath)}': ${elapsed.toFixed(2)} seconds`);
        return packageData;
    }
    finally {
...
export async function processPackagePdfWithProgress(packageId, pdfPath, destination, onProgress) {
    ...
    try {
        ...
        const itineraryPrompt = createItineraryPrompt();
        const pricingPrompt = createPricingPrompt();
        onProgress("extraction", "Running AI extraction (Marketing, Itinerary, Pricing)...", 20);
