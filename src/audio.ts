...
const ffmpegBinaryPath = ffmpegStatic as unknown as string;
...
export async function generateAudioSummary(scriptContent: string): Promise<{
  audioUrl: string;
  localPath: string; // will be empty since we delete files after upload
  tokenUsage: {
    promptTokens: number;
    outputTokens: number;
    totalTokens: number;
  ...
  try {
    for await (const chunk of response) {
      ...
      if (chunk.candidates?.[0]?.content?.parts?.[0]?.inlineData) {
        ...
        console.log(
          `Received audio data: ${inlineData.mimeType}, detected extension: ${fileExtension}`
        );

        // Ensure a WAV file first
        if (!fileExtension || fileExtension !== "wav") {
          console.log(
            `Converting ${inlineData.mimeType || "unknown"} to WAV format`
          );
          fileExtension = "wav";
          buffer = convertToWav(
            inlineData.data || "",
