...
import { GenerationConfig, PackageData, ProgressCallback } from "./types.js";
import {
...
export async function extractPackageData(
  pdfPath: string,
  ...
  try {
    ...
    const [
      ...
      executeExtraction(
        ...
        if (onProgress) onProgress("marketing", "Marketing data extracted", 30);
        return result;
      }),
      executeExtraction(
        pdfBuffer,
      ...
      executeExtraction(
        ...
        if (onProgress) onProgress("pricing", "Pricing data extracted", 65);
        return result;
      }),
    ]);
